...
import {getAllDoctors, updateDoctor } from "../../../../Actions/doctorAction";
...
const UpdateDoctorDetails = () => {
  ...
  const { message, loading, error, doctors } = useSelector(
    (state) => state.doctor
  );
  ...
  const [buttonText, setButtonText] = useState("Update Doctor");
  ...
  useEffect(() => {
    ...
    if (doctors) {
      ...
      setAvatarPreview(doctor?.avatar?.url || avatarPreviewImage);
      setOptionValueGender(doctor?.gender || "Choose Doctor Gender");
      setOptionValueAvailable(doctor?.available || "Will Doctor Available");
      setOptionValueSpeciality(
        doctor?.speciality || "Choose Doctor Speciality"
      );
    }
  }, [message, dispatch, loading, error, doctors, id]);
